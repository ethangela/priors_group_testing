...
def programming(gibbs_sample, mode='grid', skelton_h=28, skelton_w=28, optimizer='qp', num_tests=1000, beta=0.5, eta=0.006, pickle_file_path='oct09_lbd.pkl', binary=0, noise_flip=0, num_exps=1, poly=1):
    ...
    for i in range(skelton_h):
        for j in range(skelton_w):
            if binary:
                var_dic[f"x_{i}_{j}"] = model.addVar(vtype=GRB.BINARY, name=f"x_{i}_{j}")
            else:
    ...
    if not os.path.exists(pickle_file_path):
        d = {'mode':[mode], 'sample':[gibbs_sample], 'optimizer':[optimizer], 'flip':[noise_flip], 'num_tests':[num_tests], 'num_exps':[num_exps], 'objective_value':[obj.getValue()], 
            'normal_fp':[fpl[0]], 'normal_fn':[fnl[0]], 'quite_fp':[fpl[1]], 'quite_fm':[fnl[1]], 'most_fp':[fpl[2]], 'most_fn':[fnl[2]], 'binary':[binary], 'defective':[k], 'time':[comp_time], 'total_time':[total_time], 'poly':[poly], 'beta':[beta]}
...
if __name__ == '__main__':
    ...
    #                             beta=0.5, eta=0.006, binary=bi, noise_flip=flip, num_exps=exp, poly=1)
    #                         programming(gibbs_sample='grid_28_28_block_7_grid_ROUND_11_beta_0.6_ita_0.035', mode='grid_28_28_block_7', 
    #                             skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
    #                             beta=0.6, eta=0.035, binary=bi, noise_flip=flip, num_exps=exp, poly=1)
    #                     else:
    #                         programming(gibbs_sample='grid_28_28_ROUND_31_beta_0.5_ita_0.006', mode='grid_28_28', 
    #                             skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
    #                             beta=0.5, eta=0.006, binary=bi, noise_flip=flip, num_exps=exp, poly=0)
    #                         programming(gibbs_sample='grid_28_28_block_7_grid_ROUND_11_beta_0.6_ita_0.035', mode='grid_28_28_block_7', 
    #                             skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
    #                             beta=0.6, eta=0.035, binary=bi, noise_flip=flip, num_exps=exp, poly=0)

    # df = pd.read_pickle('oct09_lbd.pkl') #'to_produce_table_chart_aug19.pkl'
    # csv_name = 'chart_'+'oct09_lbd.pkl'[:-4]+'.csv'
    # df.to_csv(csv_name, index=False)




    #### oct exp 1 various lambda ####

    SKH=28
    SKW=28

    for lptp in ['lp', 'poly', 'qp']:
        for flip in [0, 0.01]: 
            for bi in [0,1]:
                for beta in [0.1, 0.3, 0.5, 0.6, 0.8, 1.0, 1.5, 2.0]:#[0.01, 0.03, 5.0, 10.0]:
                    for test in [300,500]:
                        for exp in range(50,100):
                            if lptp == 'poly':
                                programming(gibbs_sample='grid_28_28_ROUND_31_beta_0.5_ita_0.006', mode='grid_28_28', 
                                    skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
                                    beta=beta, eta=0.006, binary=bi, noise_flip=flip, num_exps=exp, poly=1)
                                programming(gibbs_sample='grid_28_28_block_7_grid_ROUND_11_beta_0.6_ita_0.035', mode='grid_28_28_block_7', 
                                    skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
                                    beta=beta, eta=0.035, binary=bi, noise_flip=flip, num_exps=exp, poly=1)
                            else:
                                programming(gibbs_sample='grid_28_28_ROUND_31_beta_0.5_ita_0.006', mode='grid_28_28', 
                                    skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
                                    beta=beta, eta=0.006, binary=bi, noise_flip=flip, num_exps=exp, poly=0)
                                programming(gibbs_sample='grid_28_28_block_7_grid_ROUND_11_beta_0.6_ita_0.035', mode='grid_28_28_block_7', 
                                    skelton_h=SKH, skelton_w=SKH, optimizer=lptp, num_tests=test, 
                                    beta=beta, eta=0.035, binary=bi, noise_flip=flip, num_exps=exp, poly=0)

    df = pd.read_pickle('oct09_lbd.pkl') #'to_produce_table_chart_aug19.pkl'
    csv_name = 'chart_'+'oct09_lbd.pkl'[:-4]+'_lambda.csv'
    df.to_csv(csv_name, index=False)
