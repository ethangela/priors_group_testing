...
def programming(gibbs_sample, mode='grid', skelton_h=28, skelton_w=28, optimizer='qp', num_tests=1000, beta=0.5, eta=0.006, pickle_file_path='apr11.pkl', binary=0, noise_flip=0, num_exps=1, poly=1):
    ...
    if mode == 'grid_28_28':
        ...
        if mode == 'grid_28_28_block_7':
            block = 7
        if mode == 'grid_84_84_block_21':
            block = 21
        adjacency = np.zeros((skelton_h*skelton_w, skelton_h*skelton_w))
        for i in range(skelton_h*skelton_w):
            for j in range(skelton_h*skelton_w):
...
if __name__ == '__main__':
    ...
    for lptp in ['poly','lp','qp']:
        for flip in [0, 0.01]: 
            for bi in [0,1]:
                    for test in range(300,2700,300):
                        for exp in range(50,55):
                            if lptp == 'poly':
                                programming(gibbs_sample='grid_84_84_block_21_grid_ROUND_3_beta_0.5_ita_0.008', mode='grid_84_84_block_21', 
    ...
    df = pd.read_pickle('apr11.pkl') #'to_produce_table_chart_aug19.pkl'
    csv_name = 'chart_'+'apr11.pkl'[:-4]+'.csv'
